...
def main():
    ...
    # Submit code or Git repository
    submit_parser = subparsers.add_parser("submit", help="Submit code or Git repository")
    ...
    if args.command == "submit":
        if args.language and args.code_file:
            ...
            # submit git repo URL from git config
            import subprocess
            try:
                ...
                exit(1)
            if not url:
                print("Error: No git remote URL found. Please provide --language and --code-file to submit code text.")
                exit(1)
            result = client.submit_git(args.problem_id, url)
